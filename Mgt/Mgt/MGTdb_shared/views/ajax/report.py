...
from ..FuncsAuxAndDb import rawQueries_report as rawQ_r
	...
	if len(request.POST) > 0:
		...
		if 'yearStart' not in request.POST or 'yearEnd' not in request.POST or int(request.POST['yearStart']) > int(request.POST['yearEnd']):
			...
			if ('country' not in request.POST or request.POST['country'] == '') and ('project' not in request.POST or request.POST['project'] == ''): # check input val
				...
				respData = rawQ_r.getDataForReport(request.user.username, request.POST['country'], request.POST['project'], True, int(request.POST['yearStart']), int(request.POST['yearEnd']), org) # fn checks if proj is user project.
		else:
			...
			# elif check yearStart and yearEnd here...
				...
				respData = rawQ_r.getDataForReport(request.user.username, request.POST['country'], None, False, int(request.POST['yearStart']), int(request.POST['yearEnd']), org)
		...
		if form.is_valid():
			...
			if request.user.is_authenticated:
				...
				if ('country' not in request.POST or request.POST['country'] == ''): # check input val
					...
					return render(request, 'Salmonella/summaryReport.html', {'dataForReport': dict_dataForReport);
