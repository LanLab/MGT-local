...
from Blankdb.views.FuncsAuxAndDb import defaults
...
def getIslnIfExists(source, type, host, disease, date, year, month):
...
class IsolationForm(forms.ModelForm):
	...
	def save(self):
		...
		islnObj = getIslnIfExists(self.instance.source, self.instance.type, self.instance.host, self.instance.disease, self.instance.date, self.instance.year, self.instance.month)
...
class IsolateForm(forms.ModelForm):
	...
	class Meta:
		...
		widgets = {
			'isQuery': forms.CheckboxInput(attrs={'onclick':"javascript:Blankdb_showHideAssembly();"})
		}
	...
	def clean(self):
		...
		if cleaned_data['file_forward'] == None and cleaned_data['file_reverse'] == None and cleaned_data['file_alleles'] == None and cleaned_data['file_assembly'] == None:
