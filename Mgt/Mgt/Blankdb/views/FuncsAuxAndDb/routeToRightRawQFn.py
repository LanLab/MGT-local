...
from Blankdb.views.FuncsAuxAndDb import rawQueries
from Blankdb.views.FuncsAuxAndDb import getPoolOfCcMergeIds as getCcMergeIdsList
from Blankdb.models import Project, User, Isolate, View_apcc, Location, Isolation, Tables_ap, Tables_cc
...
def getIsolatesFromRightFn(arr_ap, arr_cc, arr_epi, arr_loc, arr_isln, arr_iso, pageNumToGet, totalPerPage, username, isExactProj, orderBy, dir, isCsv, isMgt9Ap, searchType, isMr):
	...
	try:
		(mgtIds, locIds, islnIds, isoSearchStr, searchedProjIds, userProjIds, dict_mergedIds, isFirstSearchType) = convertSearchToIds(arr_ap, arr_cc, arr_epi, arr_loc, arr_isln, arr_iso, username, isExactProj, searchType, isFirstSearchType)
	...
	if len(searchedProjIds) > 0: # if projects are searched for
		isoCount = rawQueries.getIsolates_auth_proj_cnt(isoSearchStr, searchedProjIds, islnIds, locIds, mgtIds, searchType, isFirstSearchType)
		...
		(isolates, columns) = rawQueries.getIsolates_auth_proj(isoSearchStr, searchedProjIds, islnIds, locIds, mgtIds, offset, limit, orderBy, dir, isMgt9Ap, searchType, isFirstSearchType)



		(list_colsInfo, list_tabAps, list_tabCcs) = convertColsToDict(columns, True)
		...
		isoCount = rawQueries.getIsolates_auth_cnt(isoSearchStr, userProjIds, islnIds, locIds, mgtIds, searchType, isFirstSearchType)
		...
		(isolates, columns) = rawQueries.getIsolates_auth(isoSearchStr, userProjIds, islnIds, locIds, mgtIds, offset, limit, orderBy, dir, isMgt9Ap, searchType, isFirstSearchType)
		# print("least thought of!")

		(list_colsInfo, list_tabAps, list_tabCcs) = convertColsToDict(columns, True)
	else: # user is not logged in

		isoCount = rawQueries.getIsolates_cnt(isoSearchStr, islnIds, locIds, mgtIds, searchType, isFirstSearchType)
		...
		(isolates, columns) = rawQueries.getIsolates(isoSearchStr, islnIds, locIds, mgtIds, offset, limit, orderBy, dir, isMgt9Ap, searchType, isFirstSearchType)




		(list_colsInfo, list_tabAps, list_tabCcs) = convertColsToDict(columns, False)
...
def getMgtIds(arr_ap, arr_cc, arr_epi, searchType):
	...
	if len(arr_cc) > 0 or len(arr_epi) > 0: # cc and epi query (combine into 1)
		(andOfOrQs, dict_mergedIds) = getCcMergeIdsList.getAndOfOrQsAndMergedIds(arr_cc, arr_epi, searchType)
...
def getLocIds(arr_loc, searchType):
	qs_location = None
...
def getIslnIds(arr_isln, searchType):
	qs_isln = None
...
def makeSearchStr_retProjIdsIfSearProj(arr_iso, username, isExactProj, searchType, isFirstSearchType):
...
def convertSearchToIds(arr_ap, arr_cc, arr_epi, arr_loc, arr_isln, arr_iso, username, isExactProj, searchType, isFirstSearchType):
	...
	if not isEmptyApCcEpi(arr_ap) or not isEmptyApCcEpi(arr_cc) or not isEmptyApCcEpi(arr_epi):

		(mgtIds, dict_mergedIds) = getMgtIds(arr_ap, arr_cc, arr_epi, searchType)
	...
	if len(arr_loc) > 0 and len(arr_loc[0]) > 0:
		locIds = getLocIds(arr_loc, searchType)
	...
	if len(arr_isln) > 0 and len(arr_isln[0]) > 0:
		print(arr_isln);
		islnIds = getIslnIds(arr_isln, searchType)
	...
	if len(arr_iso) > 0 and len(arr_iso[0]) > 0:
		(isoSearchStr, searchedProjIds, isFirstSearchType) = makeSearchStr_retProjIdsIfSearProj(arr_iso, username, isExactProj, searchType, isFirstSearchType)
...
def convertColNmForDownload(list_colsInfo): # only show the display names
...
def convertColsToDict(columns, isAuth):
	# print(columns)
	...
	for col in columns:
		...
		(display_name, searchStr, isIsoFound, isLocFound, isIslnFound, isProjFound, isIdentIso, isIdentProj) =  getDisplayName(col, isIsoFound, isLocFound, isIslnFound, isAuth, isProjFound, isIdentIso, isIdentProj)
...
def getDisplayName(col, isIso, isLoc, isIsln, isAuth, isProjAdded, isIdentIso, isIdentProj):
