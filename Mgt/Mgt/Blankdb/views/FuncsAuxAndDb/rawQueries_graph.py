...
def get_timeOrLoc_StCountData(mgtIds, locIds, islnIds, isoSearchStr, searchedProjIds, userProjIds, isProjPage, projIdToExcl):
	...
	queryStr = queryStr + 'FROM "Blankdb_view_apcc" as v2 RIGHT JOIN '
	queryStr = queryStr + '('
	...
	queryStr = queryStr + 'FROM "Blankdb_isolate" as i '

	if islnIds and len(islnIds) > 0:
		queryStr = queryStr + 'INNER JOIN "Blankdb_isolation" as iM_i ON i.isolation_id = iM_i.id '
		# print("Do the isolation inner join & add a where...")
	else:
		queryStr = queryStr + 'LEFT JOIN "Blankdb_isolation" as iM_i ON i.isolation_id = iM_i.id '

	if mgtIds and len(mgtIds) > 0:
		queryStr = queryStr + 'INNER JOIN "Blankdb_view_apcc" as v ON i.mgt_id = v.mgt_id '
	else:
		queryStr = queryStr + 'LEFT JOIN "Blankdb_view_apcc" as v ON i.mgt_id = v.mgt_id '



	if locIds and len(locIds) > 0:
		queryStr = queryStr + 'INNER JOIN "Blankdb_location" as iM_l ON i.location_id = iM_l.id '
	else:
		queryStr = queryStr + 'LEFT JOIN "Blankdb_location" as iM_l ON i.location_id = iM_l.id '
	...
	(isolates, columns) = rawQueries.executeQuery_table(queryStr);
