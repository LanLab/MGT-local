...
from Blankdb.models import Tables_ap, Mgt
...
def convertToCsv_ap9(isolates, mgtId_ap9Id, dict_tabRows_byAp9Id, colNamesCombined, isGrapeTree):
...
def getTheDataMgt9Aps(isolates, list_colsInfo, isGrapeTree):
	...
	(mgtId_ap9Id, list_tabRows_byAp9Id, colNamesCombined) = getMgt9Aps_v2(theMgtIds_inIso, isGrapeTree)
...
def getTablesMgt9AsList(largestSchemeId):
...
def getTheAp9IdsFromMgt(theMgtIds, largestScheme_tn):
...
def getMgt9Aps_v2(theMgtIds, isGrapeTree):

	(largestSchemeId, largestScheme_tn) = getLargestSchemeId()

	tablesMgt9 = getTablesMgt9AsList(largestSchemeId)
	(mgtId_ap9Id, list_ap9Id) = getTheAp9IdsFromMgt(theMgtIds, largestScheme_tn)
	...
	if (len(mgtId_ap9Id) > 0 and len(list_ap9Id) > 0):
		(theAP9, columns) = buildQueryAndGetData(tablesMgt9, list_ap9Id);
		list_cols = getTheHeader(tablesMgt9);

		#print("The column are:")
		#print(columns)
		#print(theAP9)

		(idPos, toExludePositions) = getMg9ApColumnNames_v2(list_cols, isGrapeTree)
		...
		(dict_allelicProfiles, colNamesCombined) = cleanUpRows(toExludePositions, idPos, theAP9, list_cols, isGrapeTree)
...
def cleanUpRows(toExludePositions, idPos, isolates, list_cols, isGrapeTree):
...
def buildQueryAndGetData(tablesMgt9, list_ap9Id):
	queryStr = 'select'
	for i in range(0, len(tablesMgt9)):
		# queryStr = queryStr + ' "Blankdb_' + tablesMgt9[i]  + '"::text'
	...
	for i in range(0, len(tablesMgt9)):
		queryStr = queryStr + ' "Blankdb_' + tablesMgt9[i] + '" as "' + tablesMgt9[i] + '"'
...
def getTheHeader(tablesMgt9):

	print("Tables Mgt 9");
	print(tablesMgt9);
	list_cols = [];
	for i in range(0, len(tablesMgt9)):
		queryStr = 'select * from "Blankdb_' + tablesMgt9[i] + '" where false;'
...
def getMg9ApColumnNames_v2(list_cols, isGrapeTree):
...
def getMgt9Aps(theMgtIds):

	list_tabRows = list()


	(mgtId_ap9Id, list_ap9Id) = getTheAp9IdsFromMgt(theMgtIds)

	# Get the ap9 tables (ordered!)
	tablesMgt9 = getTablesMgt9AsList()

	# For each table get the rows (and add to list...(?)

	for i in range(0, len(tablesMgt9)):

		theModel = apps.get_model(app_label='Blankdb', model_name=tablesMgt9[i])
