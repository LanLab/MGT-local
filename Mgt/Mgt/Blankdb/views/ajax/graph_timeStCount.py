...
from Blankdb.views.FuncsAuxAndDb import routeToRightRawQFn_graph as routerGraph
		...
		if (sessionVar[0]['pageType'] == cl.InitialIsolates or sessionVar[0]['pageType'] == cl.InitialProjIsolates or sessionVar[0]['pageType'] == cl.SearchIsolateDetail or sessionVar[0]['pageType'] == cl.SearchIsolateList or sessionVar[0]['pageType'] == cl.SearchProjectDetail):
			...
			if request.user.is_authenticated and (sessionVar[0]['pageType'] == cl.InitialIsolates or sessionVar[0]['pageType'] == cl.SearchIsolateList or sessionVar[0]['pageType'] == cl.SearchIsolateDetail): # include user isolates (no bg strains)
				...
				if (sessionVar[0]['pageType'] == cl.SearchIsolateDetail):
					...
					(arr_ap, arr_cc, arr_loc, arr_isln, arr_iso) = ses.convertToArrs_searchIsoDetail(searchAp, searchCcEpi, searchLocs, searchIsln, searchProj, searchIso, request.user.username)
					print('Over here!');
					print(arr_loc);
				...
				(isolates, columns) = routerGraph.getDataFromRightFn(arr_ap, arr_cc, arr_epi, arr_loc, arr_isln, arr_iso, request.user.username, False, None, searchType)
			...
			# else: not logged in
				...
				if (sessionVar[0]['pageType'] == cl.SearchIsolateDetail):
					...
					(arr_ap, arr_cc, arr_loc, arr_isln, arr_iso) = ses.convertToArrs_searchIsoDetail(searchAp, searchCcEpi, searchLocs, searchIsln, searchProj, searchIso, None)
				...
				# print("The project id " + str(projectId))
				(isolates, columns) = routerGraph.getDataFromRightFn(arr_ap, arr_cc, arr_epi, arr_loc, arr_isln, arr_iso, None, False, None, searchType)
