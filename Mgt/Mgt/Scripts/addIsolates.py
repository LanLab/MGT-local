...
def handleTheFile(User, appClass, fn_isolateInfo):
		...
		with open(fn_isolateInfo, 'r', errors='replace') as fh_:
...
def makeNoneIfEmpty(val):
	...
	if val == "" or re.match('^Unknown$', val, flags=re.I) or re.match('None', val, flags=re.I):
		return None
...
def handleIsolation(appClass, dict_md_isolation, source, type, host, hostDisease, date, year, month):
	...
	if (source, type, host, hostDisease, date_formated, year, month) not in dict_md_isolation:
		...
		if iObj == None:
			iObj = addToTableInOrgDb.addIsolation(appClass, source, type, host, hostDisease, date_formated, year, month=month)
